...
;(() => {
  ...
  // now isolated to updateCurrentHangoutSession
  ...
  const startTranscribing = () => {
    ...
    setCurrentHangoutSessionDetails();
  ...
  // -------------------------------------------------------------------------
  const getTranscriptDetails = (hangoutId) => {
    ...
    for (let sessionIndex = 0; sessionIndex <= maxSessionIndex && count < MAX_PARTICIPANT_SEARCH_DEPTH; sessionIndex += 1) {
      ...
      for (let speakerIndex = 0; speakerIndex <= maxSpeakerIndex && count < MAX_PARTICIPANT_SEARCH_DEPTH; speakerIndex += 1) {
        const data = get(makeHangoutKey(hangoutId, sessionIndex, speakerIndex));
  ...
  const setSpeaker = (cache) => {
    set(makeHangoutKey(cache.hangoutId, cache.sessionIndex, cache.speakerIndex), {
      image: cache.image,
      person: cache.person,
      text: cache.text,
  ...
  const deleteHangout = (hangoutId) => {
    ...
    if (!READONLY) {
      set(KEY_HANGOUT_IDS, [...hangoutIds.slice(0, index), ...hangoutIds.slice(index + 1)]);
    }

    const transcriptNode = document.querySelector(`#${hangoutId}`);
    if (transcriptNode) {
      ...
      debug(`transcriptNode doesn't exist for ${hangoutId}`);
    }
  }
  ...
  // -------------------------------------------------------------------------
  // Find hangout name from footer
  // -------------------------------------------------------------------------
  const getHangoutName = () => {
    const name = xpath(`//*[text()='keyboard_arrow_up']/../..//div[@jscontroller!='']/text()`);

    if (name !== SEARCH_TEXT_NO_MEETING_NAME) {
      return name.data;
    }
  };

  // -------------------------------------------------------------------------
  // Identify the current hangout based on the URL. Invoked whenever we
  // start trancribing.
  // -------------------------------------------------------------------------
  const setCurrentHangoutSessionDetails = () => {
    const now = new Date();
    const dateString = `${now.getFullYear()}-${pad(now.getMonth()+1)}-${pad(now.getDate())}`;
    const pathString = document.location.pathname.match(/\/(.+)/)[1];
    const newHangoutId = `${pathString}-${dateString}`;
    const isNameChanged = newHangoutId !== currentHangoutId;

    if (isNameChanged || currentSessionIndex === null) {
      currentHangoutId = newHangoutId;

      const hangoutIds = get(KEY_HANGOUT_IDS) || [];

      if (!hangoutIds.includes(currentHangoutId)) {
        hangoutIds.unshift(currentHangoutId);
        set(KEY_HANGOUT_IDS, hangoutIds);
      }
      ...
      if (isNameChanged) {
        const name = getHangoutName();
        set(`${makeHangoutKey(currentHangoutId)}_name`, name);
      }
    }
  };
  ...
  const updateCurrentHangoutSession = (node) => {
    ...
    if (index === -1) {
      const currentSpeakerIndex = increment(makeHangoutKey(currentHangoutId, currentSessionIndex));
      CACHE.unshift({
        ...getCaptionData(node),
        startedAt: new Date(),
        endedAt: new Date(),
  ...
  const makeMenu = () => {
    ...
    const clearTranscript = () => deleteHangout(currentHangoutId);
  ...
  const getHangoutNameParts = (hangoutId) => {
    const name = get(`${makeHangoutKey(hangoutId)}_name`);
    ...
    try {
      [
        ...
        month,
        day,
      ] = hangoutId.match(/(.+)-([0-9]{4,4})-([0-9]{2,2})-([0-9]{2,2})/);
  ...
  // -------------------------------------------------------------------------
  const makeTranscriptListItem = (hangoutId) => {
    const {
      hasRecords,
      minTime,
      people,
    } = getTranscriptDetails(hangoutId);
    ...
    const _copyTranscript = tryTo(() => navigator.clipboard.writeText(getTranscript(hangoutId)), 'copying transcript');
    const _deleteTranscript = tryTo(() => deleteHangout(hangoutId), 'deleting transcript');
