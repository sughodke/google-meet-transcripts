...
;(() => {
  ...
  // -------------------------------------------------------------------------
  const syncSettings = () => {
    SPEAKER_FORMAT = getOrSet('setting.speaker-format', '**$hour$:$minute$ $name$:** $text$');
  ...
  const getTranscript = (hangoutId) => {
    ...
    for (let sessionIndex = 0; sessionIndex <= maxSessionIndex; sessionIndex += 1) {
      ...
      const sessionTranscript = speakers.join('\n');
  ...
  const setCurrentHangoutSessionLoop = () => {
    ...
    if (newHangoutId !== currentHangoutId || currentSessionIndex === null) {
  ...
  const addButtonLoop = () => {
    ...
    if (buttons && !buttons.__gmt_button_added) {
      ...
      toggleButton.onclick = tryTo(toggleTrancribing, 'toggling grid');
  ...
  console.log(`[google-meet-transcripts] localStorage version`, getOrSet('version', 1, version = null));
  setInterval(tryTo(addButtonLoop, 'adding button'), 1000);
  setInterval(tryTo(setCurrentHangoutSessionLoop, 'determining hangout id and session index'), 500);
