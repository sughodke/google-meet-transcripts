...
;(() => {
  ...
  const addButtonLoop = () => {
    try {
      ...
      if (buttons && !buttons.__grid_ran) {
        ...
        toggleButton.appendChild(makeMenuOptions());
      }
    } catch (e) {
  ...
  const makeMenuOptions = () => {
    ...
    // TODO move
    const copyTranscript = () => navigator.clipboard.writeText(getTranscript(hangoutId));
  ...
  const makeTranscript = (id) => {
    ...
    [
      ...
      make('span', `${path} on ${month}/${day}`, { onclick: _copyTranscript }),
      makeSvg(SVG_TRASH, 18, 20, { onclick: _deleteTranscript }),
      make('p', `with ${personString}`),
  ...
  setInterval(addButtonLoop, 1000);
